...
# 衛星の位置情報取得
def GetPos(self,coordinate='MSO',unit='Rm'):
    # Error 
    if not coordinate in ['MSO','MSM']:
        raise ValueError("coordinate must be MSO or MSM")
    ...
    if coordinate == 'MSO':
        ...
        positions = np.array([sp.mxv(sp.pxform("J2000", "MSGR_MSM", t), pos) for t, pos in zip(time, ptarg)])
    ...
    if coordinate == 'MSO':
        ...
        self.value['X_MSM'] = positions[:,0]
        self.value['Y_MSM'] = positions[:,1]
        self.value['Z_MSM'] = positions[:,2]
MagDataClass.MagData.GetPos = GetPos
ScanDataClass.ScanData.GetPos = GetPos

# 座標変換用メソッド
def CTransform(self,coordinate='MSM',mag=True,replace=False):
    # Error
